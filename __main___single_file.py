...
class FeatureAgent10M(FeatureAgentBase):
    ...
    def _get_vector_features(self):
        ...
        # 102-116: 其他特征
        vec[102] = self.prevalentWind / 3.0
        vec[103] = self.seatWind / 3.0
        vec[104] = sum(self.tileWall) / 84.0 if self.tileWall else 0.25
        vec[105] = 1.0 if self.wallLast else 0.0
        vec[106] = len(self.hand) / 14.0
        my_seat_pack_count = len(self.packs[self.seatWind]) if 0 <= self.seatWind < len(self.packs) else 0
        vec[107] = my_seat_pack_count / 4.0
    ...
    def _check_mahjong(self, tile, isSelfDrawn=False, isAboutKong=False):
        ...
        try:
            ...
            return sum(f[0] * f[1] for f in fans) >= 8
        except:
            return False
