...
class Actor(Process):
    ...
    def run(self):
        ...
        model = PretrainModel(hidden_dim=self.config.get('model_hidden_dim', 256), use_vec=True)
        ...
        for episode in range(self.config['episodes_per_actor']):
            ...
            while not done:
                ...
                for agent_name in obs:
                    ...
                    agent_data['state'].setdefault('vec', []).append(state['vec'])
                    obs_t = torch.tensor(state['observation'], dtype=torch.float32).unsqueeze(0)
                    vec_t = torch.tensor(state['vec'], dtype=torch.float32).unsqueeze(0)
                    mask_t = torch.tensor(state['action_mask'], dtype=torch.float32).unsqueeze(0)
                    model.train(False) # Batch Norm inference mode
                    with torch.no_grad():
                        logits, value = model(obs_t, vec_t, mask_t)
                    ...
                    agent_data.setdefault('log_prob', []).append(log_prob)
                # interact with env
